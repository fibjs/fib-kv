...
module.exports = {
    ...
    get: (conn, opts, k) => {
        var rs = exec(conn, sql(opts, 'get'), k, utils.timeout(opts) > 0 ? Date.now() - utils.timeout(opts) : undefined);
        return rs.length ? rs[0][utils.value_name(opts)] : null;
    },
    set: (conn, opts, k, v) => exec(conn, sql(opts, 'set'), k, v, utils.timeout(opts) > 0 ? Date.now() : undefined),
    has: (conn, opts, k) => exec(conn, sql(opts, 'has'), k, utils.timeout(opts) > 0 ? Date.now() - utils.timeout(opts) : undefined).length > 0,
    keys: (conn, opts) => {
        var _keys = exec(conn, sql(opts, 'keys'), utils.timeout(opts) > 0 ? Date.now() - utils.timeout(opts) : undefined).reduce((a, b) => a.concat(b instanceof Array ? b[0] : b[utils.key_name(opts)]), []);
        return _keys;
    },
