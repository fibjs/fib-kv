...
module.exports = {
    ...
    get: function(k) {
        var rs = this.conn.execute(this.sqls.get, k);
        return rs.length ? rs[0][0] : null;
    },
    set: function(k, v) {
        this.conn.execute(this.sqls.set, k, v);
    },
    has: function(k) {
        return this.conn.execute(this.sqls.has, k).length > 0;
    },
    remove: function(k) {
        this.conn.execute(this.sqls.remove, k);
    }
};
